...
function Store(props) {
  ...
  useEffect(() => {
    axiosInstance
      ...
      .then((res) => {
        setOrdersQueue(res.data.submittedOrders);
        console.log("submittedOrders", res.data.submittedOrders);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <div className="container-fluid   height">
        ...
        >
          <tbody>
            <tr key={45} style={{ width: "100%" }}>
              <th className="table-heading">{t("customer.101")}</th>
              <th className="table-heading">{t("purchased.102")}</th>
              <th className="table-heading">{t("puitems.103")}</th>
              <th className="table-heading">{t("paida.104")}</th>
              <th className="table-heading">{t("email.105")}</th>
              <th className="table-heading">{t("shiping.106")}</th>
            ...
            {ordersQueue.length !== 0 ? (
              ordersQueue.map((f, index) => {
                return f.data?.payingMethod === "Paypal" ? (
                  <tr
                    key={index}
                  ...
                  >
                    <th>
                      ...
                      <span>
                        {" "}
                        {f.data?.payerDetails?.name?.given_name +
                          f.data?.payerDetails?.name?.surname}
                      </span>
                    </th>
                    <th>
                      <span> {f.data?.purchasedItems?.length}</span>
                    </th>
                    <th className="table-heading">
                      {" "}
                      <span>
                        {" "}
                        {f.data?.purchasedItems?.map((item, index) => (
                    ...
                    <th className="table-heading">
                      ${f.data?.purchasedDetails?.amount?.value}
                    </th>
                    <th style={{ width: "0", lineBreak: "anywhere" }}>
                      {" "}
                      {f.data?.payerDetails?.email_address}
                    </th>
                    <th>
                      {" "}
                      {
                        f.data?.purchasedDetails?.shipping?.address
                          ?.address_line_1
                      }
                    </th>
                    <th>Paypal</th>
                  ...
                  <tr key={index}>
                    ...
                    <th>${f.data?.PaidAmount} </th>
                    <th style={{ width: "0", lineBreak: "anywhere" }}>
                      {f.data?.customerEmail}
                    </th>
                    <th>{f.data?.shippingAddress}</th>
                    <th>{t("stripe.84")}</th>
                  </tr>
                );
              })
              ...
              <tr>
                ...
                <td>2</td>
                <td>200</td>
                <td style={{ width: "0", lineBreak: "anywhere" }}>
                  muhammadzia785@gmail.com
                </td>
                <td style={{ width: "0", lineBreak: "anywhere" }}>
                  Qtr161/1 MRF colony PAc kamra
                </td>
                <td>paypal</td>

                <td className="d-flex justify-content-around p-10">
                  <button className="btn-done" onClick={showModalView}>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 1600 }} aria-label="sticky table" stickyHeader>
          <TableHead>
            <TableRow>
